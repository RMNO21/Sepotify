    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (intent.action) {
            ACTION_PLAY_PAUSE -> {
                ...
                } else if (state != null && state.songId.value != 0) {
                    state.updatePlayingState(!state.playingState.value)
                } else {
            ...
            CoroutineScope(Dispatchers.IO).launch {
                ...
                if (title.isNotBlank()) {
                    ...
                    if (session != null) {
                        ...
                        views.setTextViewText(R.id.widget_artist, "Tap to play latest items")
                        views.setImageViewResource(R.id.widget_play_pause, R.drawable.ic_paused)
                        views.setImageViewResource(R.id.widget_cover, R.drawable.placeholder)
                    }
                }
